...
(function () {
   ...
   */
  function spatialNavigationHandler() {
    console.log("spatialNavigationHandler");
    /*
    ...
    const candidates =
      args.candidates && args.candidates.length > 0
        ...
        : defaultCandidates.filter(
            (candidate) => container.contains(candidate) && container !== candidate,
          );
  ...
  function selectBestCandidate(currentElm, candidates, dir) {
    ...
    const spatialNavigationFunction = getComputedStyle(container).getPropertyValue(
  ...
  function navigateChain(eventTarget, container, parentContainer, dir, option) {
    ...
    if (!createSpatNavEvents("notarget", currentOption.container, eventTarget, dir))
      return;
      ...
      if (dir && typeof dir === "string") {
        ...
        if (isOverflow(element, dir)) {
          ...
          switch (dir) {
            ...
            case "down":
              return (
                overflowY !== "visible" && overflowY !== "clip" && overflowY !== "hidden"
              );
          }
        }
        return false;
  ...
  function isVisible(element) {
    return (
      !element.parentElement || (isVisibleStyleProperty(element) && hitTest(element))
    );
  }
